...
//			TemplateUtility.print(getOutput(), " M "+M+" N "+N, true);
            if (M == 0 || N == 0)
                isZeroMN = true;
...
//			TemplateUtility.print(getOutput(), "hasil", true);
            for (int i = 0; i < oddevenmap.length; i++) {
//				TemplateUtility.print(getOutput(), Arrays.toString(oddevenmap[i]), true);
...
//			TemplateUtility.print(getOutput(), "gomap - ", true);
//			for(int i=0;i<gomap.length;i++){
//				TemplateUtility.print(getOutput(), ""+Arrays.toString(gomap[i]), true);
//			}
			if(K==1){
				evenNumber = 1;
			}
...
//			TemplateUtility.print(getOutput(), "Case "+(total-T)+" "+evenNumber+" "+oddNumber, true);
        }
    }
...
//			TemplateUtility.print(getOutput(), "c "+c+" Coors "+Coors[i]+" isNotOutsideZone "+isNotOutsideZone, false);
            if (isNotOutsideZone) {
                waterZone = isWaterZone(Coors[i]);
//				TemplateUtility.print(getOutput(), " waterZone "+waterZone, false);
            }
...
//		TemplateUtility.print(getOutput(), " c "+cur+" gomap "+gomap[cur.x][cur.y]+" WATER "+WATER, true);
