...
//				System.out.println("temp "+temp);
                TemplateUtility.print(getOutput(), "#" + (i + 1) + " : " + temp, true);
                int tot = 0;
...
//			System.out.println(ans[match?1:0]);
            TemplateUtility.print(getOutput(), ans[match ? 1 : 0], true);
        }
    }

	void solve(int inputI, int digitI, int badMask){
//		System.out.println(line[inputI]+" "+badMask+" "+((line[inputI] & badMask)==0));
        TemplateUtility.print(getOutput(), inputI + " line[inputI] " + Integer.toBinaryString(line[inputI]) + " (line[inputI] & badMask) " + Integer.toBinaryString((line[inputI] & badMask)) + "", true);
        TemplateUtility.print(getOutput(), inputI + " inputI " + Integer.toBinaryString(inputI)
		...
		// more input
            ...
            TemplateUtility.print(getOutput(), " [masuk sini] "
			...
			if(((digit[digitI] ^ line[inputI])& line[inputI])==0)// new burns not revivals
				{
                    TemplateUtility.print(getOutput(), " [masuk sini#2] " + Integer.toBinaryString((line[inputI] ^ digit[digitI]) | badMask), true);
